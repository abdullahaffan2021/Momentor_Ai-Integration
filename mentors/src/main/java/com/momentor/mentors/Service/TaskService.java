...
public class TaskService {
    ...
    public List<TaskResponseDto> getalltaskdto(){
        return taskRepository.findAll()
                .stream()
                .map(task -> new TaskResponseDto(task.getId(),task.getTitle(),task.getAssignedTo(),task.getStatus(),task.isAutoGenerated(),task.isApproved()))
                .collect(Collectors.toList());
    }
    //Add Pagination
    public Page<TaskResponseDto> gettaskwithpagination(int page,int size,String sortby,String direction){
        ...
        return taskRepository.findAll(pageable).map(task -> new TaskResponseDto(task.getId(),task.getTitle(),task.getAssignedTo(),task.getStatus(),task.isAutoGenerated(),task.isApproved()));
    }
    ...
    public List<TaskResponseDto> gettaskbymentor(Long mentorId) {
        ...
        return taskRepository.findByMomIdIn(momIds).stream()
                .map(task -> new TaskResponseDto(
                        ...
                        task.getTitle(),
                        task.getAssignedTo(),
                        task.getStatus(),task.isAutoGenerated(),task.isApproved()
                ))
                .collect(Collectors.toList());
    }
