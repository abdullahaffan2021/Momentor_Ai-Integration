...
public class AiMeetingService {
    ...
    public void processmeeting(Long meetingId, String path) {
        try {
            String transcript = speechToTextService.transcribe(new File(path)); //Convert Audio -> Text With Deepgram Ai
            ...
            // Extract team names
            List<String> teamNames = teams.stream().map(Team::getName).collect(Collectors.toList()); //Collection Of Each and Every Team Name Which Present In The Team List
            //Uses Groq LLM
