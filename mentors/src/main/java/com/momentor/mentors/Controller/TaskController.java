...
public class TaskController {
    ...
    public TaskResponseDto createTask(@PathVariable Long momid, @Valid @RequestBody TaskRequestDto dto){
        ...
        return new TaskResponseDto(task.getId(),task.getTitle(),task.getAssignedTo(),task.getStatus(),task.isAutoGenerated(),task.isApproved(),task.getResources(),task.getMom().getMeeting().getType(),task.getPriority(),task.getDueDate());
    }
    ...
    public TaskResponseDto editAiTask(
        ...
        return new TaskResponseDto(
                ...
                task.isApproved(),
                task.getResources(),task.getMom().getMeeting().getType(),task.getPriority(),task.getDueDate()
        );
    }
    ...
    public ResponseEntity<?> approveTask(@PathVariable Long taskId) {
        try {
            taskService.approveAndExpandAiTask(taskId);
            return ResponseEntity.ok("Task approved and assigned to participants");
        } catch (Exception ex) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Approval failed: " + ex.getMessage());
        }
    }
    @GetMapping("/{taskId}")
    @PreAuthorize("hasRole('MENTOR')")
    ...
    @GetMapping("/alerts")
    public List<TaskAlertDto> getAlerts(Authentication authentication){

        String username = authentication.getName();

        return taskService.getDeadlineAlerts(username);
    }
    @PutMapping("/cancel/{taskid}")
    @PreAuthorize("hasRole('MENTOR')")
    public ResponseEntity<?> cancelTask(@PathVariable Long taskid,Authentication authentication){
        try{
            Task task=taskService.cancelTask(taskid,authentication);
